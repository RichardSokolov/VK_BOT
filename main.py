...
class VKBot:
        ...
        try:
            ...
            if date is not None:
                ...
                if len(date_list) == 3:
                    ...
                    self.write_msg(user_id, 'Не смог узнать ваш возраст, пожалуйста задайте параметры поиска')
                    self.write_msg(user_id, 'Введите нижний порог возраста (мин - 16): ')
        ...
        try:
            ...
            if date is not None:
                ...
                if len(date_list) == 3:
                    ...
                    self.write_msg(user_id, 'Введите верхний порог возраста (макс - 65): ')
                    for event in self.longpoll.listen():
                        if event.type == VkEventType.MESSAGE_NEW and event.to_me:
                            age = event.text
                            return age
    ...
    def find_persons(self, user_id, offset):
        ...
        self.send_photo_1(user_id, 'Фотография номер 1', offset)
        if self.get_photo_2(self.person_id(offset)) != None:
            self.send_photo_2(user_id, 'Фотография номер 2', offset)
            self.send_photo_3(user_id, 'Фотография номер 3', offset)
        else:
    ...
    def found_person_info(self, offset):
        ...
        return f'{list_person[0]} {list_person[1]}, ссылка - {list_person[3]}, приятного общения!'
